...
plot_steps_per_day <- function(steps_per_day, mean_steps, median_steps) {
    ...
    ggplot(steps_per_day, aes(x=steps)) + 
        geom_histogram(fill="#FF6666", binwidth=1000) + 
        geom_point(aes(x=mean_steps, y=0, color="green"), size=5, shape=20) + 
        geom_point(aes(x=median_steps, y=0, color="yellow"), size=5, shape=20) + 
...
plot_activity_pattern <- function(steps_per_interval, max_step_interval) {
    ...
    ggplot(steps_per_interval, aes(x=interval, y=steps)) +   
        ...
        geom_point(aes(x=max_step_interval, y=0, color="red"), size=5, shape=25) +  
        scale_color_manual(name=element_blank(), labels=col_labels, values=cols) +     
...
activity_with_impute_data <- data.frame(  
    steps = imp_means(activity, steps_per_interval),  
    date = activity$date,  
